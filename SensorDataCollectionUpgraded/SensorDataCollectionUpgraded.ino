...
void loop() {
  ...
  if ((motion == true) || (state.lastLampStatus != state.currentLampStatus) || (prevLoggedBrightness != loggedBrightness) || ( abs(prevAmbientLight - ambientLight) == AMBIENT_LIGHT_THRESHHOLD )){ // if these parameterrs change, send data
